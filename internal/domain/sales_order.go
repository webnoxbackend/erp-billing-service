...
func (so *SalesOrder) CanCreateInvoice() bool {
	// Allow invoice creation for confirmed orders or shipped orders without an invoice (data inconsistency fix)
	return (so.Status == SalesOrderStatusConfirmed || so.Status == SalesOrderStatusShipped) && so.InvoiceID == nil
}
